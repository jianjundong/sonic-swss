        ...
        ProducerStateTable *p_acl_rule_tbl, std::map<std::string,std:: string> *p_isolate, char *msg)
{
    ...
    if (found != string::npos)/*vxlan tunnel*/
    {
        ...
        std::string pre = "VXLAN_TUNNEL|";
        std::string key = pre + vxlan_tnl_name;
        std::unordered_map<std::string, std::string>::iterator attr_src_ip;
        std::unordered_map<std::string, std::string>::iterator attr_dst_ip;

        auto hash = p_redisClient_2_cfg->hgetall(key.c_str());

        attr_src_ip = hash.find("src_ip");
        if (attr_src_ip != hash.end())
        {
            FieldValueTuple src_ip("SRC_IP",attr_src_ip->second);
            acl_rule_attrs.push_back(src_ip);
        }
        
        attr_dst_ip = hash.find("dst_ip");
        if (attr_dst_ip != hash.end())
        {
            FieldValueTuple dst_ip("DST_IP",attr_dst_ip->second);
            acl_rule_attrs.push_back(dst_ip);
        }

        FieldValueTuple ip_protocol_attr("IP_PROTOCOL","17");/*UDP*/
        acl_rule_attrs.push_back(ip_protocol_attr);
        
        FieldValueTuple dst_port_attr("L4_DST_PORT","4789");
        acl_rule_attrs.push_back(dst_port_attr);
...
void mclagsyncd_set_port_mac_learn_mode(ProducerStateTable *p_port_tbl, ProducerStateTable *p_tnl_tbl, 
        ProducerStateTable *p_lag_tbl, std::map<std::string,std:: string> *p_learn, char *msg)
{
    ...
    for (index= 0; index < res.size(); ++index)
    {
        ...
        else if(strncmp(res[index].c_str(),"VTT",3)==0)
    ...
    { 
        ...
        switch (hdr->msg_type)
        {
            ...
            case MCLAG_MSG_TYPE_PORT_MAC_LEARN_MODE:
                    mclagsyncd_set_port_mac_learn_mode(p_port_tbl, p_tnl_tbl, p_lag_tbl, p_learn, msg);
                    break;
 ...
 void mclag_connection_lost_handle_port_Isolate(RedisClient *p_redisClient_2_cfg)
{
  ...
  void mclag_connection_lost_handle_port_learn_mode(std::map<std::string,
        std:: string> *p_learn,  ProducerStateTable * p_port_tbl, ProducerStateTable * p_lag_tbl, ProducerStateTable * p_tnl_tbl)
 {
     vector<FieldValueTuple> attrs;
     std::map<std::string,std:: string>::iterator it;
     ...
     while (it != p_learn->end())
     {
        ...
        else if(strncmp(it->first.c_str(),"VTT",3) == 0)
 ...
 void mclag_connection_lost( std::map<std::string,std:: string> *p_isolate, std::map<std::string,std:: string> *p_learn, 
            ProducerStateTable * p_port_tbl, ProducerStateTable * p_lag_tbl, ProducerStateTable * p_tnl_tbl, RedisClient *p_redisClient_2_cfg)
 {
     mclag_connection_lost_handle_port_Isolate(p_redisClient_2_cfg);     
     /*mclag_connection_lost_handle_port_Isolate(p_isolate, p_port_tbl);*/
     mclag_connection_lost_handle_port_learn_mode(p_learn, p_port_tbl, p_lag_tbl, p_tnl_tbl);
     return;
 }
