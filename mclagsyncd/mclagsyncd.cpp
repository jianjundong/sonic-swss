...
extern  void mclag_connection_lost( std::map<std::string,std:: string> *p_isolate, std::map<std::string,std:: string> *p_learn, 
ProducerStateTable * p_port_tbl, ProducerStateTable * p_lag_tbl, ProducerStateTable * p_tnl_tbl, RedisClient *p_redisClient_2_cfg);
extern void mclag_test(ProducerStateTable &port_tbl);
...
int main(int argc, char **argv)
{
    ...
    DBConnector cfg_db(CONFIG_DB, DBConnector::DEFAULT_UNIXSOCKET, 0);
    ...
    ProducerStateTable acl_tbl(&cfg_db, CFG_ACL_TABLE_NAME);
    ProducerStateTable acl_rule_tbl(&cfg_db, CFG_ACL_RULE_TABLE_NAME);
    RedisClient redisClient_2_asicDb(&asic_db);
    RedisClient redisClient_2_appDb(&appl_db);
    RedisClient redisClient_2_countersDb(&counters_db);
    RedisClient redisClient_2_cfgDb(&cfg_db);
    ...
    while (1)
    {
        try
        {
            ...
            mclag.p_redisClient_2_app = &redisClient_2_appDb;
            mclag.p_redisClient_2_counters = &redisClient_2_countersDb;
            mclag.p_redisClient_2_cfg = &redisClient_2_cfgDb;
        ...
        {
            mclag_connection_lost(&isolate, &learn_mode, &port_tbl, &lag_tbl, &tnl_tbl, &redisClient_2_cfgDb);
